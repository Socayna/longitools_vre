...
function generate_RSA_keys($account,$password=""){
	
	$keys_dir= $GLOBALS['dataDir']."/".$_SESSION['User']['id']."/.keys/".$_SESSION['User']['activeProject']."$account";

	$private = RSA::createKey();
	$public = $private->getPublicKey();
	#$private_enc = $private->withpassword("bsccns");
	$private_openssh= $private->toString('OpenSSH', ['comment' => $_SESSION['User']['id']."@".$_SERVER['SERVER_NAME'] ]);
	$public_openssh = $public->toString('OpenSSH', ['comment' => $_SESSION['User']['id']."@".$_SERVER['SERVER_NAME'] ]);

	echo "<br/>PRIVATE<br/>" . $private;
	echo "<br/>PUBLIC<br/>" . $public;
	echo "<br/>PRIVATE OPENSSH<br/>" . $private_openssh;
	echo "<br/>PUBLIC OPENSSH<br/>" . $public_openssh;
	echo "</br>";
	echo "<br/> $keys_dir/rsa_openssh.priv";


	file_put_contents("$keys_dir/rsa_openssh.priv", $private_openssh);
	file_put_contents("$keys_dir/rsa_openssh.pub", $public_openssh);
	#chmod($this->getPrivateKeyFile(), 0600);

	$private2 = PublicKeyLoader::load(file_get_contents("$keys_dir/rsa_openssh.priv"));
	echo "<br/>PRIVATE 2 <br/>" . $private2;
	$public2 = PublicKeyLoader::load(file_get_contents("$keys_dir/rsa_openssh.pub"), $password = "");
	echo "<br/>PUBLIC 2 <br/>" . $public2;
