...
function InputTool_printSelectFile($input, $rerun, $ff, $multiple, $required) {
	...
	if(!$multiple) {
		...
		$output = '<div class="form-group">
			<label class="control-label">'.$input['description'].$labelopt.' <i class="icon-question tooltips" data-container="body" data-html="true" data-placement="top" data-original-title="<p align=\'left\' style=\'margin:0\'>'.$input['help'].'</p>"></i></label>
...
function InputTool_printSelect($input) {
	...
	if($input["type"] == "boolean") {
		...
		if (!isset($input['enum_items']['description'])){
			$input['enum_items']['description'] = $input['enum_items']['name']; 
		}
		$tool_options = $input['enum_items'];
	}
	...
	$output = '<div class="form-group">
				...
				for ($i=0; $i<count($tool_options['name']); $i++) {
					...
					if(($default != "") && ($tool_options['name'][$i] == $default)) $sel = "selected";
					else $sel = "";
					$output .= '<option value="'.$tool_options['name'][$i].'" '.$sel.'>'.$tool_options['description'][$i].'</option>';
				}
	$output .= '</select>
